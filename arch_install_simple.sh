...
# Modified for: Dual boot with Windows (using partitions 5, 6 and 7)
...
# Define devices (assuming partitions are already created)
DISK="/dev/nvme0n1"
EFI_DEV="${DISK}p5"  # Partition 5: EFI System (1GB)
SWAP_DEV="${DISK}p6"  # Partition 6: Linux swap (8GB)
SYSTEM_DEV="${DISK}p7"  # Partition 7: Linux filesystem (440GB)
...
# --- 13) INSTALL ESSENTIAL PACKAGES FIRST ---
print_message "Instalando paquetes esenciales del sistema..."
pacman -S --noconfirm \
    networkmanager sudo grub efibootmgr os-prober ntfs-3g mtools dosfstools \
...
# --- 29) CONFIGURE HYPRLAND, WAYBAR AND ALACRITTY ---
...
#!/bin/bash
# Script to randomly change wallpaper using hyprpaper
# This script changes wallpaper by selecting a random image from ~/Wallpapers directory

WALLPAPERS_DIR="$HOME/Wallpapers"

if [ ! -d "$WALLPAPERS_DIR" ] || [ -z "$(ls -A $WALLPAPERS_DIR)" ]; then
    notify-send "Error" "No wallpapers available in $WALLPAPERS_DIR" -i dialog-error
    exit 1
fi
...
notify-send "Wallpaper changed" "New wallpaper: $RANDOM_WALLPAPER" -i dialog-information
EOF

chmod +x /home/antonio/.config/hypr/scripts/wallpaper-changer.sh

# Basic hyprpaper configuration
cat > /home/antonio/.config/hypr/hyprpaper.conf << EOF
# To add wallpapers, place images in ~/Wallpapers and add the following lines:
# preload = /home/antonio/Wallpapers/your-image.jpg
# wallpaper = HDMI-A-1,/home/antonio/Wallpapers/your-image.jpg
# wallpaper = eDP-1,/home/antonio/Wallpapers/your-image.jpg
EOF
...
cat > /home/antonio/.config/waybar/config.jsonc << 'EOF'
{
    ...
    "modules-center": [],
    "modules-right": ["custom/cpu", "custom/gpu", "custom/ram", "battery", "network", "custom/volume", "bluetooth", "clock"],
...
To install Visual Studio Code and additional tools:
...
3. Configure Rust correctly:
   rustup default stable

4. For projects with Tauri:
   sudo npm install -g @tauri-apps/cli

5. Verify language installations:
   - PHP: php --version
...
IMPORTANT NOTES:
- Alacritty is configured with the same shortcuts you had in Kitty
- Ctrl+C/V work for copy/paste
- Ctrl+K clears the screen
- Ctrl+Shift+C/V/K send the original Bash signals
- To switch between US and ES keyboard: Alt+Shift
- To change wallpaper: Ctrl+Super+T
- For screenshot: Print key or Super+Z
- Wofi opens with Super+R (replacement for Rofi)
- Volume control: VOL module in Waybar shows current percentage
- Click on VOL to open pavucontrol for detailed volume settings

ABOUT SECURITY:
- Steam and Discord only transmit data to their own servers
- Bluetooth is installed but you can disable it if not using:
  sudo systemctl disable bluetooth
- For more security, install a firewall:
  sudo pacman -S ufw
  sudo ufw enable
...
# Change ownership of files to antonio user
chown -R antonio:antonio /home/antonio/

print_message "Base installation has been completed."
print_message "After reboot:"
print_message "1. Remove installation media"
print_message "2. Select Arch Linux or Windows in GRUB"
print_message "3. If you select Arch, log in as 'antonio'"
print_message "4. Connect to Internet with 'nmtui' if necessary"
print_message "5. Review the file ~/recomendaciones-post-instalacion.txt for more information"
print_message "6. To switch between English (US) and Spanish keyboard, use Alt+Shift"
print_message "7. Volume percentage is displayed in Waybar (VOL module)"
EOL

# Make post-chroot script executable
