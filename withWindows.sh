...
# Configuration: Dual boot - Arch Linux + Windows (separate EFI partitions)
# Partition layout:
#   Partition 5: ~1GB EFI System (for Arch GRUB)
#   Partition 6: ~499GB Linux Filesystem (BTRFS)
#   No swap (32GB RAM)
...
if [ ${#PARTITIONS[@]} -lt 2 ]; then
    print_error "Expected at least 2 partitions for Arch (EFI + System)"
...
print_warning "DUAL BOOT: Select ONLY the Arch-dedicated partitions."
print_warning "Do NOT select the Windows EFI (200MB) or Windows NTFS partition."
echo

read -p "Enter partition number for ARCH EFI (~1GB): " efi_choice
read -p "Enter partition number for BTRFS SYSTEM (~499GB): " sys_choice

# Validate input is numeric and within range
for choice in "$efi_choice" "$sys_choice"; do
...
# --- SAFETY CHECK: Prevent selecting Windows partitions ---
print_warning "You selected:"
print_warning "  EFI:    $EFI_DEV"
print_warning "  SYSTEM: $SYSTEM_DEV"
echo
read -p "Are these correct? This will FORMAT these partitions. (yes/no): " confirm
if [ "$confirm" != "yes" ]; then
...
print_message "Formatting Arch EFI partition (${EFI_DEV})..."
...
pacman -S --noconfirm networkmanager sudo grub efibootmgr os-prober ntfs-3g mtools dosfstools nano vim git linux-headers
...
# --- 26) CONFIGURE GRUB FOR DUAL BOOT ---
print_message "Configuring GRUB for dual boot..."
...
print_success "GRUB configured with os-prober for dual boot."
...
DUAL BOOT INFO:
- Windows and Arch use SEPARATE EFI partitions
- GRUB with os-prober detects Windows automatically
- To boot Windows: select it from GRUB menu
- If Windows is not detected, run: sudo grub-mkconfig -o /boot/grub/grub.cfg
...
NO SWAP CONFIGURED:
- System has 32GB RAM, swap is unnecessary
- If needed later: sudo fallocate -l 8G /swapfile && sudo chmod 600 /swapfile
...
print_warning "DUAL BOOT: Select Arch Linux or Windows from the GRUB menu."
